...
export const Screen03Atrangi: React.FC = () => {
  const content = storyData.chapters.screen03;
  ...
  return (
    <section className="relative min-h-[100svh] w-full flex flex-col justify-center px-6 py-24 bg-[#080808] snap-start overflow-hidden">
      ...
      <div
        className="absolute top-1/3 left-1/2 -translate-x-1/2 w-[340px] h-[340px] rounded-full pointer-events-none opacity-20 blur-[110px] bg-[#581825]"
        aria-hidden="true"
      />
      ...
      <div className="max-w-md mx-auto w-full flex flex-col items-center text-center relative z-10">
        ...
        {/* Visual Transition: Two delicate lines converging toward each other */}
        <div className="relative w-full max-w-xs h-12 flex items-center justify-center my-4">
          <motion.div
            initial={{ width: 0, opacity: 0 }}
            whileInView={{ width: '45%', opacity: 0.6 }}
            viewport={{ once: true }}
            transition={{ duration: 1.2, ease: [0.22, 1, 0.36, 1], delay: 0.2 }}
            className="absolute left-0 h-[1px] bg-gradient-to-r from-transparent to-[#F5F2EC]"
          />
          ...
          <motion.div
            ...
            className="relative z-10 p-2 rounded-full bg-[#141414] border border-white/10 text-[#A7A39B]"
          >
            <Scissors className="w-3.5 h-3.5 text-[#F5F2EC]/80" />
          </motion.div>
          ...
          <motion.div
            ...
            viewport={{ once: true }}
            transition={{ duration: 1.2, ease: [0.22, 1, 0.36, 1], delay: 0.2 }}
            className="absolute right-0 h-[1px] bg-gradient-to-l from-transparent to-[#F5F2EC]"
          />
        </div>

        {/* Atrangi Video in Scrapbook Frame */}
        {content.video && (
          <motion.div
          ...
          >
            ...
            <div
              className="absolute -top-3 left-1/2 -translate-x-1/2 z-20 w-24 h-5 bg-white/[0.12] backdrop-blur-md border border-white/20 rounded-[1px] transform -rotate-[1deg] pointer-events-none"
              style={{
                backgroundImage:
            ...
            <div className="relative p-3 pb-5 rounded-[4px] bg-[#141414]/95 border border-white/15 shadow-[0_20px_50px_rgba(0,0,0,0.85)] backdrop-blur-xl">
              ...
              <div className="relative overflow-hidden rounded-[2px] bg-black aspect-[16/9] border border-black/80">
                ...
                >
                  {content.video.fallbackSrc && (
                    <source src={content.video.fallbackSrc} type="video/quicktime" />
                  )}
                  Your browser does not support the video tag.
                </video>

                {/* Subtle vignette shadow on video edges */}
        ...
        {/* Story Sentences Card */}
        <motion.div
          initial={{ opacity: 0, y: 16 }}
          whileInView={{ opacity: 1, y: 0 }}
          viewport={{ once: true }}
          transition={{ duration: 0.8, delay: 0.6 }}
          className="w-full max-w-sm p-6 rounded-2xl bg-[#121212]/80 backdrop-blur-xl border border-white/10 shadow-xl space-y-3 mt-4"
        >
          {content.storyLines.map((line, idx) => (
            <p
              ...
              className={`font-serif leading-relaxed ${
                idx === 1
                  ? 'text-base sm:text-lg text-[#F5F2EC] font-normal'
        ...
        <motion.div
          ...
          className="mt-8 px-5 py-3 rounded-lg bg-[#181818]/90 border border-white/10 shadow-lg relative transform -rotate-1"
        >
          <div className="absolute -top-2 left-1/2 -translate-x-1/2 w-8 h-2.5 bg-white/10 rounded-[1px]" />
